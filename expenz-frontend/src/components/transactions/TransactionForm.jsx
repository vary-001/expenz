...
const TransactionForm = ({ onSubmit, loading, initial = null, onCancel }) => {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    if (validate()) {
      onSubmit({ ...form, amount: parseFloat(form.amount), type: 'expense' });
    }
  };
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      <Input label="Description" name="description" value={form.description}
             onChange={handleChange} error={errors.description} placeholder="What did you spend on?" />
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        ...
        <Select label="Category" name="category" value={form.category}
                onChange={handleChange} error={errors.category} options={expenseCategories} />
      </div>
      <Input label="Date" name="date" type="date" value={form.date}
             onChange={handleChange} error={errors.date} icon={CalendarIcon} />
      <Input label="Notes (optional)" name="notes" value={form.notes}
